"""The five ``segment.sh -o`` artefacts, written only when an output folder is given."""
...
def write_artifacts(
    ...
    """Write exactly the five artefacts; ``scene_json`` is the exact stdout payload."""
    ...
    atomic_write_bytes(out_dir / "segments.ply", ply_bytes(segments, comment="oh-my-slam segments"))
